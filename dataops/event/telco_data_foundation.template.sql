-- ============================================================================
-- Cubic³ Connected Vehicle AI Demo - Data Foundation (DataOps Template)
-- ============================================================================
-- Description: Creates tables and loads Cubic³ connected vehicle connectivity, commercial, and operational data (legacy table names retained for demo compatibility)
...
) COMMENT = 'Product categories for Cubic³ SDV connectivity services (in-car data, infotainment/StreamLocal³, fleet payments, OTA/telematics, enterprise backhaul)';
...
CREATE OR REPLACE TABLE product_dim (
    ...
    vertical VARCHAR(50)
) COMMENT = 'Cubic³ offerings: SDV connectivity plans, infotainment bundles, OTA/telematics services, fleet payments, enterprise backhaul';
...
CREATE OR REPLACE TABLE customer_dim (
    ...
    state VARCHAR(10),
    zip VARCHAR(20)
) COMMENT = 'Cubic³ customers across OEM programs, fleet operators, enterprise/backhaul partners, and channel partners';
...
) COMMENT = 'Cubic³ organizational departments';
...
CREATE OR REPLACE TABLE sales_rep_dim (
    ...
    hire_date DATE
) COMMENT = 'Cubic³ sales and partner success representatives';
...
CREATE OR REPLACE TABLE employee_dim (
    ...
    gender VARCHAR(1),
    hire_date DATE
) COMMENT = 'Cubic³ employees for HR analysis';
...
CREATE OR REPLACE TABLE job_dim (
    ...
    job_level INT
) COMMENT = 'Job titles and levels within Cubic³';

-- Location Dimension (HR)
CREATE OR REPLACE TABLE location_dim (
    location_key INT PRIMARY KEY,
    location_name VARCHAR(200) NOT NULL
) COMMENT = 'Cubic³ offices, NOCs, and data centres';

-- ============================================================================
-- Step 3: Create Fact Tables
-- ============================================================================

-- Cubic³ KPI Reference (demo)
CREATE OR REPLACE TABLE cubic_kpi (
    metric VARCHAR(200) PRIMARY KEY,
    value NUMBER(18,2),
    as_of_note VARCHAR(200),
    category VARCHAR(50)
) COMMENT = 'Reference KPIs for Cubic³ demo (fleet/coverage, availability, reach, financing)';
...
) COMMENT = 'Illustrative ARPA/ARPU by segment for Cubic³ demo';
...
-- Load Cubic³ KPI Reference
COPY INTO cubic_kpi
FROM @{{ env.EVENT_DATA_STAGE | default('DATA_STAGE') }}/demo_data/cubic_kpi.csv
FILE_FORMAT = CSV_FORMAT
ON_ERROR = 'CONTINUE';
...
SELECT 'Cubic³ Connected Vehicle AI Demo data foundation complete!' AS status,

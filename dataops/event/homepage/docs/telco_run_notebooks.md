...
This notebook loads Cubic³-connected vehicle data from stages and processes it with Cortex AI.
...
- Loads 6 CSV files from `@raw_files` stage:
  - `network_performance.csv` (49,864 records) for connectivity KPIs
  - `customer_details.csv` (10,000 vehicle/driver profiles)
...
- Uses `AI_TRANSCRIBE` to convert driver support audio to text
...
- Processes Cubic³ product guides, runbooks, and policy docs
...
- Search for partner/competitor mentions (e.g., Orange, Vodafone, AT&T)

...
ALTER SESSION SET QUERY_TAG = '''{"origin":"sf_sit-is", "name":"Cubic3 Connected Vehicle AI Demo", "version":{"major":1, "minor":0},"attributes":{"is_quickstart":0, "source":"sql"}}''';

-- ============================================================================
-- Create Warehouse Early (required for EXECUTE IMMEDIATE blocks)
-- ============================================================================
CREATE WAREHOUSE IF NOT EXISTS {{ env.EVENT_WAREHOUSE | default('TELCO_DEMO_WH') }}
...
GRANT USAGE ON WAREHOUSE {{ env.EVENT_WAREHOUSE | default('TELCO_DEMO_WH') }} TO ROLE {{ env.EVENT_ATTENDEE_ROLE | default('TELCO_ANALYST_ROLE') }};
GRANT OPERATE ON WAREHOUSE {{ env.EVENT_WAREHOUSE | default('TELCO_DEMO_WH') }} TO ROLE {{ env.EVENT_ATTENDEE_ROLE | default('TELCO_ANALYST_ROLE') }};
...
CREATE DATABASE IF NOT EXISTS {{ env.EVENT_DATABASE | default('TELCO_OPERATIONS_AI') }}
    COMMENT = 'Database for Cubic³ Connected Vehicle AI demo - connectivity data, documents, and apps pre-loaded';
...
CREATE OR REPLACE SCHEMA {{ env.EVENT_SCHEMA | default('DEFAULT_SCHEMA') }}
    COMMENT = 'Main schema for Cubic³ demo data tables';
...
CREATE OR REPLACE NETWORK RULE telco_web_access_rule
  ...
  COMMENT = 'Permissive network access for Cubic³ demo web scraping';
...
ALTER USER {{ env.EVENT_USER_NAME }} SET
    ...
    DEFAULT_WAREHOUSE = {{ env.EVENT_WAREHOUSE | default('TELCO_DEMO_WH') }};
...
SELECT 'Account configuration complete!' AS status,
       ...
       '{{ EVENT_WAREHOUSE | default("TELCO_DEMO_WH") }}' AS warehouse_created,
...
SELECT 
    ...
    '{{ EVENT_WAREHOUSE | default("TELCO_DEMO_WH") }}' as warehouse_name,
